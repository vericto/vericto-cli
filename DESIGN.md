...
> see §1/§13), **new repo `vericto-cli`**, **Rust**, **HTTP/JSON transport** (gRPC
...
> chunk concurrency (§5), project-level `.vericto.toml` config (§6), system +
...
`vericto` is a command-line client for validating SQL against a Vericto workspace's
...
locally. It sends SQL to the Vericto backend and reports the verdict.

```
migration.sql ──► vericto check ──HTTPS──► POST /api/v1/ci/check-key ──► engine
                      │                                                    │
                      └────────────── verdict + exit code ◄───────────────┘
```
...
- **Response:**
  ```json
  {
    ...
    "queries": [ {
      ...
      "rule_code": "VERICTO-001", "ast_node_path": "DeleteStmt > WhereClause = NULL",
...
```
vericto check [files...]     Evaluate SQL files (or stdin with '-'). The core command.
vericto login                Browser login (default, §6.0), or --api-key (verified, §6.0.1) / --oidc (§6.1).
vericto logout               Remove stored credentials.
vericto doctor               Verify config, connectivity, auth, and plan entitlement.
vericto baseline [files...]  Record current findings to .vericto-baseline.json (§10).
vericto baseline prune       Remove stale baseline entries that no longer match (§10).
vericto rules list           List the workspace's effective rule catalogue (§15).
vericto rules show <CODE>    Show one rule's detail, incl. its AST condition (§15).
vericto init                 Scaffold CI workflow + pre-commit hook (§10); --oidc for workload-identity.
vericto verify-receipt <f>   Verify a signed run receipt offline (§7.1).
vericto version              Print the CLI version (same as --version).
vericto completions <shell>  Print a shell completion script (bash|zsh|fish|powershell|elvish).
```
...
| `--timeout <secs>` | `30` | Per-request timeout (`VERICTO_TIMEOUT`) — §6 |
| `--concurrency <n>` | `4` | Max in-flight chunk requests, capped at 8 (`VERICTO_CONCURRENCY`) — §5 |
| `--api-key <k>` | env/config | Override stored key |
| `--api-url <u>` | `https://api.vericto.com` | Override backend |
...
| `--workspace <id>` | env/config | Workspace to authenticate against for OIDC (`VERICTO_WORKSPACE_ID`) — §6.1 |
| `--audience <aud>` | `vericto` | OIDC audience to request in the ID token — §6.1 |
| `--oidc-token-env <VAR>` | `VERICTO_ID_TOKEN` | Env var holding a pre-minted OIDC ID token (GitLab-style) — §6.1 |
| `--ca-bundle <path>` | — | Extra trusted CA PEM bundle (`VERICTO_CA_BUNDLE` / `SSL_CERT_FILE`) — §6.4 |
...
- **Chunk concurrency (🔜 — point 7):** a large monorepo with thousands of
  ...
  `--concurrency <n>` / `VERICTO_CONCURRENCY` (capped at 8 to stay a well-behaved
...
2. Env: `VERICTO_API_KEY`, `VERICTO_API_URL`
3. Project config: `.vericto.toml` at the repo root (🔜 — see §6.3)
...
`vericto login` stores the key; `vericto doctor` validates it and reports the
remaining monthly CLI allowance (surfacing an exhausted-quota or scope problem
clearly rather than a raw 4xx at check time).

`vericto login` itself is now three mutually exclusive modes, dispatched on
which flags are given:

| Invocation | Mode | Where it's for |
|---|---|---|
| `vericto login` (no flags) | Browser login (§6.0) | A developer at a keyboard with a browser — the default |
| `vericto login --api-key <key>` | Verified manual entry (§6.0.1) | Scripts, headless boxes, providers without OIDC |
| `vericto login --oidc --workspace <id>` | Workload-identity config (§6.1) | CI providers with OIDC support — no secret ever stored |

### 6.0 Browser login (✅ — this revision, "verified login")

**Problem.** The original `vericto login --api-key` (or its interactive prompt)
had no positive signal that the pasted value was even a valid key — the first
signal was whatever the next `check` run returned, which for most users is
mid-CI-pipeline, not at `login` time. A raw key also has no natural place to
apply the fail-closed principle "an unverified credential is never persisted"
*before* the flow existed to verify it against anything.

**Design:** the same loopback + one-time-code pattern `gcloud auth login` /
`aws sso login` / `gh auth login` use for local human login — chosen over
returning the key directly to the browser (a plain redirect with the key in
the URL) because a one-time code keeps the actual credential out of browser
history, any proxy/log sitting between the browser and the dashboard, and the
page's own JS-visible fetch responses:

1. `vericto login` generates a random `state` (32 bytes, CSPRNG) and binds a
   loopback TCP listener on `127.0.0.1:0` (OS-assigned free port — never a
   fixed port, so concurrent `login` runs or another process never collide).
2. It opens `{app_url}/cli-auth?state=<state>&port=<port>` in the default
   browser (`open` / `xdg-open` / `cmd /c start`, OS-dependent, best-effort —
   printing the URL is the fallback if launching fails).
3. The dashboard's `/cli-auth` page (already authenticated via the normal
   session cookie — no credential typed here) lets the user pick a workspace
   and calls `POST /api/v1/auth/cli-login/approve`. The backend mints a
   **30-day**, `ci_dryrun:execute`-scoped key immediately, but does **not**
   return it — only a short-lived (120s), single-use exchange `code`.
4. The browser does a **top-level navigation** (not a fetch — no CORS surface
   to configure on either side) to `http://127.0.0.1:<port>/callback?state=
   ...&code=...`.
5. The CLI's loopback server accepts that one connection, checks `state`
   matches what it generated in step 1, and extracts `code`. A mismatched
   `state` is rejected and the server keeps waiting (see the "what `state`
   defends against" note below) — bounded by a 5-minute overall deadline.
6. The CLI exchanges `code` for the plaintext key via
   `POST /api/v1/auth/cli-login/exchange` — a server-to-server call, no
   browser involved, authenticated purely by possession of the code.
7. The key is saved to `~/.config/vericto/config.toml` exactly like the manual
   path (§6.0.1) — same file, same `0600` permissions, same shape.

**What `state` does and does not defend against.** The backend never inspects
`state` — standard OAuth convention: it is generated by the CLI, opaque to the
authorization side, carried through the browser, and re-checked only by the
CLI itself against its own value. It does not need to defend against a
malicious dashboard (the backend already scopes/expires the minted key
independently, and the exchange code is single-use — see
`backend/src/services/cli-login.ts`). What it *does* defend against is a
different flow on the same machine — another `login` run, or an unrelated
local process — hitting this process's loopback port with an unrelated code
while it's waiting; without the check, that stray hit could otherwise be
mistaken for the real callback.

**Revocation:** the key is named `"CLI (browser login)"` in the dashboard's
API Keys list (like any other key) and can be revoked or left to expire at the
30-day mark — no different from a hand-created key with `expires_in_days` set.

**Implemented (this revision).** CLI side: `src/browser_login.rs`
(`generate_state`, `LoopbackServer::bind`/`wait_for_callback`, `open_browser`);
`api::cli_login_exchange` posts `{ code }` to `/api/v1/auth/cli-login/exchange`.
`run_login_browser` in `main.rs` drives the sequence above and calls the same
`save_login` helper the manual path uses. Backend side:
`services/cli-login.ts` (`approveCliLogin` mints the key + stores it under the
code via a new Redis consume-once helper — `setOnce`/`consumeOnce`, `SET ... EX
NX` / `GETDEL` — in `db/redis.ts`; `exchangeCliLoginCode` redeems it exactly
once), `routes/auth.ts` `POST /cli-login/approve` (JWT-gated, checks workspace
membership before minting) and `POST /cli-login/exchange` (unauthenticated,
code-gated). Frontend: `web/app/cli-auth/page.tsx` (workspace picker + approve
button, mirrors the existing invitation-accept page's auth-gate pattern) and
`apiClient.auth.cliLoginApprove`.

Flags: `--app-url <url>` (dashboard origin to open; defaults to
`DEFAULT_APP_URL`), plus the usual `--api-url`/`--timeout`/`--ca-bundle`
shared with every network-calling command.

### 6.0.1 Verified manual entry (`--api-key`) — fail-closed (✅ — this revision)

**Problem.** Even with browser login covering the interactive case, a raw
`--api-key <key>` (scripts, headless CI, providers without OIDC) was still a
blind write: whatever string was passed got saved to disk with zero
verification. A typo'd, revoked, wrong-workspace, or wrong-scope key would
only surface on the first `check` — silently trusting an unverified credential
is the opposite of what a security-focused CLI should default to.

**Design:** `vericto login --api-key <key>` now calls
`GET /api/v1/ci/config` — the same read-only, quota-free pre-flight endpoint
`check` and `doctor` already use — **before** writing anything. There is no
escape hatch:

- **Auth failure** (401/403 — bad, revoked, or wrong-scope key): nothing is
  saved, exit `3`.
- **Backend unreachable, or any other error** (`Transport`/`Backend`): nothing
  is saved either, exit `4`. A network blip does not get treated differently
  from a bad key — an unverified credential is never persisted, full stop, no
  `--skip-verify`/`--force` flag to bypass it. This mirrors the CLI's existing
  fail-closed defaults elsewhere (§5's chunk failures, §14's discussion of why
  `--allow-degraded` is scoped narrowly and never touches auth).
- **Success:** the workspace's plan is printed as a confirmation, then the key
  is saved exactly as before.

**Implemented (this revision).** `run_login_with_key` in `main.rs`, sharing
the `save_login` helper with the browser path. No backend change was needed —
`GET /api/v1/ci/config` already existed for `check`'s pre-flight sanitization
check (§6.2) and `doctor`.
...
**Design:** `vericto login --oidc` (or auto-detected when `ACTIONS_ID_TOKEN_REQUEST_TOKEN`
...
4. The CLI holds this token in memory for the process lifetime only; it is
   never written to `~/.config/vericto/config.toml` or any file.
...
`VERICTO_ID_TOKEN`, overridable with `--oidc-token-env`). `api::oidc_exchange`
...
static key exists but a CI token is available. `vericto login --oidc --workspace
<id>` stores the workspace/audience (never a secret) and, when run inside CI,
verifies the exchange live. `vericto init --oidc --workspace <id>` scaffolds
OIDC-flavored CI templates (GitHub `id-token: write` + `--oidc`; GitLab
`id_tokens:` + `--oidc`), with no `VERICTO_API_KEY` secret. Backend side was
...
Flags: `--oidc`, `--workspace <id>` (or `VERICTO_WORKSPACE_ID` / `.vericto.toml`
`workspace_id` / config), `--audience <aud>` (default `vericto`),
`--oidc-token-env <VAR>`.

### 6.2 Query sanitization — inherits the workspace setting (🔜 — point 2)

`vericto-proxy` already has `TelemetryQueryMode` (`Raw` | `Sanitized` — literals
...
- The `/api/v1/ci/check-key` response (and the dashboard-side ruleset sync the
  ...
  `telemetry_query_mode` (`raw` | `sanitized`) — the same value `vericto-proxy`
...
- Today, `sql_preview` in the response is already server-truncated/rendered —
  ...
  *before* sending (same normalization the engine/proxy already do — see
  `vericto-eval`'s literal handling), so raw literal values never leave the
...
mysql --fail-on flag` in every pipeline YAML. `.vericto.toml`, committed at the
...
```toml
# .vericto.toml — committed to the repo, PR-reviewable
default_dialect = "mysql"
fail_on = "flag"
baseline = ".vericto-baseline.json"   # relative to repo root
```

No credentials are ever allowed in this file (`vericto check` rejects `api_key`
if present here, pointing at `vericto login` / env / OIDC instead) — it's
...
**Design:**
- `--ca-bundle <path>` / `VERICTO_CA_BUNDLE`: an additional PEM bundle trusted
...
- **Timeout:** per-request timeout defaults to 30s, overridable with
  `--timeout <secs>` / `VERICTO_TIMEOUT` for slow links or large batches.
...
With `--fail-on` gating a pipeline, a `api.vericto.com` outage — even a short
one — fails every gated pipeline for every customer simultaneously. Today the
only escape is removing the `vericto check` step from the pipeline by hand,
...
- `--allow-degraded` (env: `VERICTO_ALLOW_DEGRADED=1`): when the backend is
  ...
  - **Implemented:** the record is appended as one JSON line to
    `.vericto/degraded-runs.jsonl` (`kind`/`version`/`unix_time`/`reason`/`files`/
...
- Off by default. Turning it on is a deliberate per-invocation (or per-pipeline
  env var) choice, never a config-file default — `.vericto.toml` (§6.3) rejects
...
- **SQL is user data, not a secret to the CLI — but it leaves the machine.** By
  design the SQL is sent to the customer's own Vericto tenant for evaluation (§13).
...
- **text** (default): colorized, one block per finding + a summary footer.
  ```
  ✖ migration.sql:12  BLOCKED  VERICTO-001 (critical)  DELETE without WHERE
...
- `vericto verify-receipt <file>` verifies **offline** (no network, no account):
  ...
  verify. `--public-key <PEM|path>` overrides the key (or `VERICTO_RECEIPT_PUBLIC_KEY`).
...
- The customer's own pipeline archives this file as a build artifact
  ...
  Vericto's plan tier. The dashboard's hosted history remains the convenient,
...
degraded-run record (`.vericto/degraded-runs.jsonl`); a stderr warning still
...
> `@vericto/vericto-cli`; publishing needs `NPM_TOKEN`), Homebrew/Scoop 🔜.
...
- **Phase 2 (adoption): npm ✅, Homebrew + Scoop 🔜.** Cover the Node ecosystem,
  ...
  - **npm — done (this revision):** `cargo-dist`'s `npm` installer
    (`installers = ["shell", "npm"]`, `npm-scope = "@vericto"`) generates the
    `@vericto/vericto-cli` package whose `postinstall` downloads the matching
    prebuilt binary from the GitHub release and installs the `vericto` shim. Worth
    it because CI runners overwhelmingly already have Node, so
    `npx @vericto/vericto-cli check` is zero-install. The pipeline *builds* the
    ...
    setup left. (Package name is `@vericto/vericto-cli`, derived from the crate;
    not `@vericto/cli`.)
...
- **Backend compatibility check (✅ — implemented).** The CLI fails clearly on
  ...
  sets an `X-Vericto-Api-Version` header on `/ci/check-key` (both present in
...
undercut the growth loop §12.1 depends on (annotations are what take Vericto from
"one dev's terminal" to "the whole team's review").

### GitHub

- **Action:** a thin composite action wrapping the binary; `vericto check` on the
  ...
  annotations. `vericto init` can scaffold this workflow.
...
- Wired via `artifacts:reports:codequality:` (and `:sast:`) in `.gitlab-ci.yml`;
  `vericto init` can scaffold this too.
...
- Composes with hand-rolled patterns too: `git diff --name-only --diff-filter=d
  $BASE...HEAD -- '*.sql' | vericto check --stdin-file-list` (✅ implemented) reads
...
- `vericto baseline` writes a `.vericto-baseline.json` capturing the *current* set of
  findings (by stable fingerprint: rule code + normalized AST path + file, **not**
  line number, so edits elsewhere don't shift it).
- `vericto check --baseline .vericto-baseline.json` reports baselined findings as
  informational and **does not fail** on them; only *new* findings above
  `--fail-on` trip the exit code.
- Inline suppression: a `-- vericto:ignore[VERICTO-001] reason` comment on/above a
...
- **`vericto baseline prune [files...] [--file <baseline>] [--dry-run]`** (✅ —
  this revision) is the action `check --baseline`'s drift note was pointing at:
  it re-runs the same check and removes entries whose fingerprint no longer
  matches any current finding (`baseline::drifted`), rather than leaving stale
  cleanup as a manual JSON edit. `--dry-run` lists what would be removed
  without writing. An empty baseline or a baseline with no stale entries is a
  no-op (exit 0, no unnecessary network call for the former). The file set
  passed should match what the baseline was originally recorded against —
  pruning against a narrower set (e.g. only `--changed` files) would treat
  entries for un-rechecked files as stale, which is a different claim than
  "this was fixed". A subcommand of `baseline` rather than a new top-level verb,
  since it operates on the same file/dialect/auth flags `baseline` already
  takes and clap resolves `vericto baseline prune ...` vs. the flat
  `vericto baseline [files...]` form unambiguously (subcommand vs. positional
  files).

### Pre-commit & generic

- **Pre-commit hook:** `vericto init --hook` writes a `.git/hooks/pre-commit` (or a
  `.pre-commit-config.yaml` entry) that runs `vericto check --changed` on staged SQL.
- **Generic:** any pipeline can just run `vericto check migrations/ && deploy`.
...
- **v0.2 — DONE:** the "make it a real team tool" release —
  ...
  - Integration: GitHub Action **and** GitLab CI templates; `vericto init`
    (hook + workflow scaffolding, `--oidc` variant) (§10).
  - Changed-files selection: `--changed` / `--since` / `--stdin-file-list` (§10).
  - Baseline + inline suppression: `vericto baseline`, `--baseline`,
    `-- vericto:ignore[...]` (§10).
...
- **Enterprise-readiness — DONE (this revision):** OIDC/workload-identity login
  ...
  `.vericto.toml` (§6.3), CA trust (§6.4), degraded-mode break-glass (§6.5, with a
  local audit record; server-side reconciliation decided against), bounded chunk
  concurrency
  (§5), backend compatibility check (§9), `--no-color`, `vericto version`,
  `vericto completions <shell>` (bash/zsh/fish/powershell/elvish).
...
- **`vericto baseline prune` — DONE (this revision):** removes stale baseline
  entries (§10) — the action the existing drift note pointed at but didn't
  perform.
- **Verified login — DONE (this revision):** browser-based login (§6.0, the new
  default for `vericto login`) and fail-closed verification of a manually
  provided `--api-key` (§6.0.1) — an unverified credential is never persisted
  either way.
- **Still 🔜:** distribution Homebrew/Scoop; npm registry publish (`NPM_TOKEN`);
  keyless attestations (repo must be public/org); watch/dev ergonomics. Backend
  now exposes API-key-gated CI run history (`GET /api/v1/ci/reports[/:id]`) —
  a future `vericto reports list`/`show` CLI command could consume it, but
...
1. **Plan gating — RESOLVED (strategy below).** The CLI is available on **every
   ...
   **How it avoids cannibalization — the tier map** (✅ = implemented today,
   ...
   | SARIF → PR annotations, `vericto init` | basic 🔜 | ✅ 🔜 | ✅ 🔜 | ✅ 🔜 |
   ...
   **Refinements to the raw strategy (deliberate design calls):**
   ...
   - **SARIF: give the *format* free, charge the *integration convenience*.**
     ...
     sees Vericto", so gating the raw output would throttle the growth that drives
     Team conversions. Plan: `--format sarif` output is free; the polished
     GitHub Action + `vericto init` scaffolding + custom rules are where Builder+
...
- **Requires network egress** to the customer's Vericto backend (air-gapped CI is
  unsupported — §12.4).
- **SQL leaves the machine** — sent to the customer's own Vericto tenant for
  evaluation.
...
and forcing every customer's deploys to halt during a Vericto-side outage with
...
2. **Never silent** — a reason is mandatory, a stderr warning always prints,
   and the bypass writes a local `.vericto/degraded-runs.jsonl` record (reason +
...
deploys when Vericto is down?" and "nothing, ever, no matter what" is not
...
Vericto backend; sanitization (§6.2) still requires a workspace policy fetched
...
## 15. Rule catalogue: `vericto rules list` / `vericto rules show <CODE>`

### Motivation

A CI failure only surfaces a rule code (`VERICTO-001`) and an AST path — a
developer who doesn't already know the catalogue by heart has to leave the
terminal and open the dashboard to understand what the rule actually checks,
what severity it carries, and what action it resolves to under the workspace's
policy. That round-trip is friction the thin-client design otherwise avoids
everywhere else (§1: the whole point of the CLI is not needing the dashboard
for a day-to-day gate). `rules list`/`rules show` close that loop without
introducing a second source of truth: both read the **same** effective
ruleset `evaluateCiCheck` scores a `check` run against (override-merged
standard rules + active custom rules), so what the CLI prints can never drift
from what a `check` run actually does.

### Backend contract (new, additive)

Two new read-only endpoints on the existing CI API-key surface — no new scope,
no new key needed for an existing CI integration:

- **`GET /api/v1/ci/rules`** — the workspace's effective rule catalogue.
  ```json
  {
    "rules": [ {
      "code": "VERICTO-001", "name": "DELETE without WHERE",
      "description": "Blocks DELETE with no WHERE clause",
      "severity": "critical", "dialect": "all", "rule_type": "standard",
      "is_active": true, "resolved_action": "block"
    } ],
    "ruleset_version": "v1.0.0-20260711"
  }
  ```
- **`GET /api/v1/ci/rules/:code`** — one rule's full detail (matched
  case-insensitively), a superset of the list item adding
  `ast_condition_yaml` — the exact condition the engine evaluates.
- **Auth:** same as `check-key` — API key via `X-API-Key`/`Bearer`, scope
  `ci_dryrun:execute`. An existing CI key that already runs `vericto check`
  works with `rules list`/`rules show` unchanged.
- **Not metered:** neither call touches `assertCiQuota` — read-only lookups
  don't spend the monthly CLI-check allowance (§12.1).
- **Shared resolution logic:** `resolved_action` is computed by
  `services/rule-policy.ts` (`resolveAction`), extracted from what was
  previously duplicated between the dashboard's `routes/rules.ts` and the
  telemetry rule-sync endpoint. `routes/ci.ts`'s new handlers reuse the same
  function, so "what action does this severity resolve to" has exactly one
  implementation across the dashboard, the proxy sync, and the CLI.

### CLI design

- **`vericto rules list [--active-only] [--format text|json]`** — text is a
  column-aligned table (code, severity, resolved action, active/inactive,
  name); `--active-only` filters client-side (the backend always returns the
  full catalogue, since a developer investigating "why did this used-to-work
  rule stop firing" needs to see inactive rules too).
- **`vericto rules show <CODE> [--format text|json]`** — text prints the full
  detail block including the AST condition YAML, indented for readability.
  An unknown code is a **usage error (exit 2)**, not a backend error (exit 4)
  — the lookup succeeded, the input was just wrong, mirroring how `check`
  treats a bad flag vs. an unreachable backend.
- **Auth/transport flags** (`--api-key`, `--api-url`, `--oidc`, `--workspace`,
  `--audience`, `--oidc-token-env`, `--timeout`, `--ca-bundle`) are the exact
  same shape and resolution precedence as `doctor` (§6), sharing
  `resolve_auth` — no separate auth path to maintain.
- **`--format json`** mirrors the backend response verbatim (plus the
  filtering `--active-only` already applied) for scripting — same "server is
  the source of truth for shape, client only renders" principle as `check`
  (§7).

### Non-goals

- **No rule authoring here.** `rules list`/`show` are read-only, matching the
  existing non-goal "no rule authoring UI" (§1) — creating/editing rules stays
  a dashboard (or `POST .../rules/custom`) operation, not a CLI one.
- **No local caching.** Every invocation is a live network call, consistent
  with the thin-client decision (§13) — the catalogue can change (an admin
  toggles a rule) and the CLI should never show a stale answer.
